...
int main(int argc, char *argv[])
{
	...
	void *taboo_list, *rand_taboo_list, *walk_taboo_list;
	srand(time(NULL));
	bool backtrack_flag = false, break_flag = false, regenerate_flag = false;
	int rand_i, rand_j;
	int last_best = 10000, last_best_ever;
	int rand_list[5000][2];
	int rand_list_index = 0;
	int memory[500][2];
	int memory_index = 0;
	int walk_memory[500][2];
	int walk_memory_index = 0;
...
/*		new_g = (int *) malloc((gsize + 1) 
	...
	walk_taboo_list = FIFOInitEdge(100);
	...
	/*		for(i = 0; i < gsize; i ++) {
		...
		while(try_count > 0) {
			...
			for(i = 0; i < gsize; i ++) {
         	...
         	g[i * gsize + j] = 1 - g[i * gsize + j];
					count = CliqueCountUseCache(g, gsize, i, j, best_count);
					if(count != -1 && !FIFOFindEdgeCount(walk_taboo_list, i, j, 100)) {
           	if(count < best_count) {
             	best_count = count;
             	best_i = i;
             	best_j = j;
			...
			FIFOInsertEdgeCount(walk_taboo_list, best_i, best_j, 100);
			printf("WALKING (%d, %d) best_count %d \n", best_i, best_j, best_count);	
			try_count --;
		}
		PrintIntermediateGraph(g, gsize);
		}
		//}
